...
# if not os.path.exists(path_base):
#     # dl1 machine
#     path_base = "/home/yu/datasets"
# if not os.path.exists(path_base):
#     path_base = "/ml/home/ml-pschramowski/datasets"


def run_epoch(split, upto=None):
    proc_name = "Yu-MADE:%d:%d-sine-lr%.8f" % (epoch, args.epoch, args.learning_rate)
...
#
#
# def loss_gaussian2(out, x):
#     n = x.shape[1]
#     miu = out[:, 0:n]
#     std = out[:, n:]
#     gaussian_model = tdist.Normal(miu, std)
#     log_p = gaussian_model.log_prob(x)
#     # log_density = - torch.log(std) - 0.5 * torch.mul((x-miu)/std, (x-miu)/std)
#
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lr', '--learning-rate', type=float, default=0.018,
                        help="Learning rate")
    ...
    data_train = np.fromfile(path_base + 'train_sine.dat',
                             dtype=np.float64).reshape(-1, 204)
    data_pos = np.fromfile(path_base + 'test_sine_positive.dat',
                           dtype=np.float64).reshape(-1, 204)
    data_neg = np.fromfile(path_base + 'test_sine_negative.dat',
                           dtype=np.float64).reshape(-1, 204)
